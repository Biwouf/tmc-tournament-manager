...
| `components/LiveMatchCard.tsx` | `LiveScorePage` | Carte d'un match (V1 TV Board). Hero bar avec bloc court coloré contextuel (rouge LIVE / sombre Finished / ambre Pending), chips `type_tournoi` + Simple/Double, badge statut compact + menu kebab (Démarrer/Reprendre/Voir + Supprimer). Scoreboard ATP avec trophée vainqueur, classement (chip) et club par joueur, cellules de score 48×48 (set gagné fond sombre). Footer date + "Live · en cours" ou "Vainqueur : …". Badge "À supprimer" intégré au hero bar. |
