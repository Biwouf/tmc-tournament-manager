...
| `components/LiveScoreEntry.tsx` | `LiveMatchPage` | Interface +/- par joueur pour saisir le score (sets 1-3, tiebreak auto à 6/6, choix format set 3). Désactivé si `status != 'live'` ou si la prop `forceDisabled` est `true` (utilisée par `LiveMatchPage` lors d'une perte de contrôle live). La cellule de jeu utilise `AnimatedScoreCell` (flip vertical + halo rouge) sur le set actif. |
| `components/AnimatedScoreCell.tsx` | `LiveScoreEntry`, `LiveMatchCard` | Cellule de jeu animée : flip vertical 2 phases (400ms scaleY) sur changement de valeur + halo rouge pulsant (700ms). `animate` désactive l'effet pour les sets terminés/inactifs. Reproduite à l'identique dans `pwa/src/components/matches/AnimatedScoreCell.tsx` — à maintenir synchronisée. Keyframe `scoreCellRingPulse` défini dans `src/index.css` (et PWA). |
...
| `components/matches/MatchCard.tsx` | Carte avec actions conditionnelles : Démarrer / Reprendre+Libérer / **Prendre le contrôle** (live d'un autre user — ouvre une modale de warning avec le nom du gestionnaire actuel, lu dans `profilesMap`) / Voir+Supprimer (selon auth + ownership). Cellules de score via `AnimatedScoreCell` (mini 28px, flip + halo sur la cellule du set en cours uniquement). |
| `components/matches/AnimatedScoreCell.tsx` | Copie PWA de `src/components/AnimatedScoreCell.tsx`. À maintenir synchronisée. Keyframe `scoreCellRingPulse` défini dans `pwa/src/index.css`. |
