...
| `pages/InvitePage.tsx` | `/admin/invite` | Formulaire admin pour inviter un nouvel utilisateur au BO. Deux actions : « Envoyer l'invitation » (email via Supabase) ou « Générer un lien à copier (sans email) » — fallback utile en cas de rate limit SMTP ou pour partage manuel. **PR4** : sélecteur de rôle (`member` par défaut — le moins privilégié) et envoi du `clubId` courant (`useClub()`) à l'Edge Function ; message dédié quand la function répond `already_existed` (compte préexistant rattaché au club, sans email). Route protégée par le guard auth **et** réservée aux `admin`/super-admin. |
...
| `lib/invite.ts` | **Multi-tenant (PR5)** — `invokeInvite(body)`, appel de l'Edge Function `invite-user` **extrait** de `InvitePage` pour être partagé avec `SuperAdminPage`. Porte le parsing d'erreur non trivial (lire le vrai message dans le body d'une réponse non-2xx, `invokeErr.message` étant générique) : ne pas le dupliquer. Contrat PR4 : `{ email, redirectTo, action, club_id, role }`. |
