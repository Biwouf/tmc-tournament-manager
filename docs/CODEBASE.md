...
| `contexts/ClubContext.tsx` | **Multi-tenant (PR2)** — `ClubProvider` + hook `useClub()`. Résout le club courant (tenant) depuis le hostname (`<slug>.feelike.app`, sinon `VITE_DEV_CLUB_SLUG`, fallback `cac-tennis`) via un `select` sur `clubs`, expose `{ clubId, club, loading }`. Toutes les requêtes métier filtrent par `clubId` (`.eq('club_id', clubId)` / `club_id: clubId`). Dupliqué en `pwa/src/contexts/ClubContext.tsx` (comme `liveScoreRules.ts`). |
