  ...
  return (
    <div className="flex items-center justify-center gap-3">
      <button
        ...
        onClick={onDec}
        disabled={!canDec}
        className="h-11 w-11 rounded-lg border border-border bg-card text-xl font-semibold text-foreground transition hover:bg-muted disabled:opacity-30"
      >
        −
      </button>
      <div className="w-10 text-center font-mono text-2xl font-bold tabular-nums text-foreground">{value}</div>
      <button
        type="button"
        onClick={onInc}
        disabled={!canInc}
        className="h-11 w-11 rounded-lg border border-primary/30 bg-primary/10 text-xl font-bold text-primary transition hover:bg-primary/20 disabled:opacity-30"
      >
        +
      </button>
    </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {winner && (
          <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700">
            {winner === 'j1' ? team1Label : team2Label}
          </span>
        )}
      </div>
      ...
      {showTb && set.tb_j1 !== null && set.tb_j2 !== null && (
        <div className="mt-4 border-t border-border pt-3">
          ...
          <div className="grid grid-cols-2 gap-3 text-center">
            <div className="font-mono text-lg font-semibold tabular-nums text-foreground">{set.tb_j1}</div>
            <div className="font-mono text-lg font-semibold tabular-nums text-foreground">{set.tb_j2}</div>
          </div>
          <p className="mt-2 text-xs text-muted-foreground">
            Les boutons +/- pilotent le tiebreak tant que le set est à 6/6.
          </p>
        </div>
      )}
    </div>
...
function SuperTbRow({
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <div className="flex items-center gap-2">
        <h4 className="text-sm font-semibold text-foreground">Set 3 — Super Tiebreak</h4>
        {winner && (
          <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700">
            {winner === 'j1' ? team1Label : team2Label}
          </span>
...
export default function LiveScoreEntry({ match, onPatch, forceDisabled = false }: Props) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3 px-4 text-center text-xs font-semibold uppercase tracking-wide text-muted-foreground">
        ...
        <div className="rounded-xl border border-amber-300 bg-amber-50 p-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="min-h-11 flex-1 rounded-lg border border-border bg-card px-3 py-2 text-sm font-medium transition hover:bg-muted"
            >
              Set normal
            </button>
            <button
