  ...
  return (
    <section className="shell section [--sec-top:84px]" aria-labelledby="news-title">
      ...
      <div className="grid auto-rows-fr items-stretch gap-6 md:grid-cols-2">
        {feeds.news.map(item => (
          <article key={item.id} className="relative flex min-w-0 flex-col overflow-hidden rounded-card border border-line bg-card shadow-soft transition-colors hover:bg-brand-soft focus-within:outline-2 focus-within:outline-brand">
            {item.image && <img src={item.image} alt="" loading="lazy" decoding="async"
              className="aspect-video w-full shrink-0 object-cover" style={focalPointStyle(item.focal)} />}
            <div className="flex flex-1 flex-col p-6 break-words">
              ...
              {item.excerpt && <p className="truncate text-[15px] leading-relaxed text-muted">{item.excerpt}</p>}
              <span className="mt-auto inline-block pt-4 text-sm font-bold text-brand">Lire la suite <span aria-hidden="true">→</span></span>
            </div>
          </article>
        ))}
      </div>
