...
const source = (await readFile(new URL('../supabase/functions/signup-email-dispatch/index.ts',import.meta.url),'utf8'))
  .replace(/^import .*;$/gm,'');
function harness({ code=201, attempts=1, missing=false, network=false, lookup=false, body='Votre inscription est validée.', settingsError=false, deliveryError=false, brand={ color:'#123456', logo:'https://club.example/logo.png' }, cronSecret='dedicated-cron-secret-at-least-32-characters' }={}) {
 let handler; let claims=0; const updates=[]; const sends=[]; const reads=[];
 const env={SIGNUP_EMAIL_CRON_SECRET:cronSecret,SUPABASE_SERVICE_ROLE_KEY:'server-secret',SUPABASE_URL:'https://example.test',
 ...
 const db={rpc:async()=>{claims++;return {data:[{id:'job',claim_token:'token',user_id:'member',
  title:'Inscription validée',body,club_name:'Club',attempts}]};},
 auth:{admin:{getUserById:async()=>({data:{user:{email:'member@example.test'}},error:lookup?new Error():null})}},
 from:(table)=>({
  select:(columns)=>{
   const filters={};
   const result=()=>{reads.push({table,columns,filters});return table==='signup_email_deliveries'
 ...
 return {run:(auth='dedicated-cron-secret-at-least-32-characters',method='POST')=>handler(new Request('https://example.test',{
  method,headers:{authorization:'Bearer independently-validated-gateway-jwt','x-signup-email-secret':auth}})),updates,sends,reads,get claims(){return claims;}};
}
...
test('signup HTML uses the claimed club identity and preserves plain text',async()=>{
 const h=harness(); await h.run();
 const body=h.sends[0].body;
 assert.equal(body.subject,'Inscription validée');
 assert.equal(body.textContent,'Votre inscription est validée.');
 assert.match(body.htmlContent,/#123456/);
 assert.match(body.htmlContent,/https:\/\/club.example\/logo.png/);
 assert.match(body.htmlContent,/Votre inscription est validée/);
...
test('signup email escapes user text and preserves line breaks',async()=>{
