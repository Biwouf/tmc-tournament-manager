...
Deno.serve(async (req) => {
  ...
  const cronSecret = Deno.env.get('SIGNUP_EMAIL_CRON_SECRET');
  if (!cronSecret || cronSecret.length < 32) {
    return new Response('Cron authentication not configured', { status: 503 });
  }
  if (req.headers.get('x-signup-email-secret') !== cronSecret) {
    return new Response('Unauthorized', { status: 401 });
  }
  const url = Deno.env.get('SUPABASE_URL');
  const apiKey = Deno.env.get('BREVO_API_KEY');
  const from = Deno.env.get('SIGNUP_FROM_EMAIL') || Deno.env.get('CONTACT_FROM_EMAIL');
  if (!serviceKey || !url || !apiKey || !from) return new Response('Not configured', { status: 503 });
  const db = createClient(url, serviceKey);
  const { data, error } = await db.rpc('signup_email_claim', { p_limit: 10 });
  ...
  for (const job of data || []) {
    ...
    try {
      ...
      if (!account.user?.email) {
        ...
        // L’identité visuelle reste liée au club de la demande.
        const { data: delivery, error: deliveryError } = await db.from('signup_email_deliveries')
        ...
        const rendered = renderEmail({ name: job.club_name, color: brand?.color, logo: brand?.logo }, {
          title: job.title,
          paragraphs: [job.body],
          footer: 'Cet email concerne votre club et une demande d’inscription.',
        });
        ...
        if (!response.ok) {
          ...
          console.error('Signup email rejected', job.id, response.status);
        }
        await response.body?.cancel();
      }
    } catch {
      status = 'pending';
      console.error('Signup email delivery interrupted', job.id);
    }
    if (status === 'pending' && job.attempts >= 5) status = 'failed';
    const { error: finishError } = await db.from('signup_email_deliveries').update({
